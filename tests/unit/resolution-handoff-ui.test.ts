...
import {
  resolutionHandoffView,
  type ResolutionHandoffViewInput
} from "@/lib/ui/resolution-handoff";
...
describe("resolution handoff view", () => {
  it("is READY after validated evidence and before closure persistence", () =>
    expect(resolutionHandoffView(base()).mode).toBe("READY"));
  it("is RESOLVED only with both summaries and both closure audit events", () => {
    ...
    value.audit.push(
      { type: "RESOLUTION_HANDOFF_CREATED", timestamp: "2026-09-03T09:00:03.000Z" },
      { type: "INCIDENT_RESOLVED", timestamp: "2026-09-03T09:00:03.001Z" }
    );
