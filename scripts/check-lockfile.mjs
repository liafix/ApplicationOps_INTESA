...
const errors = [];
if (lock.lockfileVersion !== 3) errors.push(`lockfileVersion expected 3, got ${lock.lockfileVersion}`);
...
if (lock.version !== pkg.version) errors.push(`lock version expected ${pkg.version}, got ${lock.version}`);
for (const section of ['dependencies', 'devDependencies']) {
  const expected = pkg[section] ?? {};
  const actual = root?.[section] ?? {};
  for (const [name, version] of Object.entries(expected)) {
    if (actual[name] !== version) errors.push(`${section}.${name} expected ${version}, got ${actual[name] ?? '<missing>'}`);
  }
  for (const name of Object.keys(actual)) {
    if (!(name in expected)) errors.push(`${section}.${name} present in lock root but absent from package.json`);
  }
}
const exact = {
...
for (const [name, version] of Object.entries(exact)) {
  ...
  else if (entry.version !== version) errors.push(`${name} resolved version expected ${version}, got ${entry.version}`);
}
if (errors.length) {
