...
function baseInput(
  status: "READY_FOR_VALIDATION" | "VALIDATED" = "READY_FOR_VALIDATION"
): import("../lib/ui/recovery-validation").RecoveryValidationViewInput {
  return {
    incident: {
      status,
...
assert(
  recoveryEvidencePassCount(evidence) === 4,
  "4/4 recovery evidence conditions derive from persisted state"
);
const readyView = recoveryValidationView(ready);
assert(
  readyView.mode === "READY",
  "READY_FOR_VALIDATION + complete evidence + pending checks exposes READY gate"
);
assert(
  readyView.persistedPassCount === 0,
  "evidence readiness does not manufacture persisted PASS statuses"
);
...
assert(
  validatedView.mode === "VALIDATED",
  "VALIDATED requires 4/4 persisted PASS plus validation audit evidence"
);
...
assert(
  recoveryValidationView(tampered).mode === "BLOCKED",
  "tampered recovery request blocks validation readiness"
);
...
assert(
  recoveryValidationView(timeoutRegression).mode === "BLOCKED",
  "new timeout ERROR after rollback blocks validation readiness"
);

const missingAudit = baseInput("VALIDATED");
missingAudit.validation = missingAudit.validation.map((check) => ({
  ...check,
  status: "PASS" as const
}));
assert(
  recoveryValidationView(missingAudit).mode === "INCONSISTENT",
  "validated state without validation audits fails closed"
);
