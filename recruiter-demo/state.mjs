...
export function runPrimaryAction(input) {
  ...
  switch (state.step) {
    ...
    case 2: {
      if (state.incidentStatus !== 'INVESTIGATING') throw new Error('Regression confirmation requires INVESTIGATING.');
      if (!(SCENARIO.observedLatencyMs > SCENARIO.currentTimeoutMs && SCENARIO.currentTimeoutMs < SCENARIO.previousTimeoutMs)) {
    ...
    case 3: {
      if (state.incidentStatus !== 'REGRESSION_CONFIRMED' || state.rootCause !== 'RELEASE_TIMEOUT_REGRESSION') {
    ...
    case 4: {
      if (state.incidentStatus !== 'REMEDIATION_SELECTED' || state.remediation !== 'ROLLBACK_RELEASE') {
      ...
      state.recoveryRequest = { id: SCENARIO.recoveryRequestId, status: 200, release: SCENARIO.previousRelease };
      state.recoveryTransaction = { id: SCENARIO.recoveryTransactionId, status: 'SUCCEEDED', release: SCENARIO.previousRelease };
      addAudit(state, 'ROLLBACK_COMPLETED', 'Presentation engine', '07:36:01');

      const validationReady = state.activeRelease === SCENARIO.previousRelease &&
        state.serviceHealth === 'HEALTHY' &&
    ...
    case 5: {
      if (state.incidentStatus !== 'VALIDATED' || state.validation.some((item) => item.status !== 'PASS')) {
      ...
      state.businessSummary = 'A release-level timeout regression caused elevated synthetic transaction failures. The previous stable release was restored, recovery checks passed 4/4, and the synthetic incident was closed after validation.';
