...
export interface ResolutionHandoffViewInput extends ResolutionHandoffInput {
  incident:
    | (NonNullable<ResolutionHandoffInput["incident"]> & {
...
export function resolutionHandoffView(input: ResolutionHandoffViewInput) {
  ...
  const hasSummaries = Boolean(
    input.incident?.closureTechnicalSummary && input.incident?.closureBusinessSummary
  );
  ...
  if (status === "RESOLVED") {
    const consistent =
      derived.ready && hasSummaries && summariesMatch && resolvedAudit && handoffAudit;
    return {
      ...derived,
      mode: consistent ? ("RESOLVED" as const) : ("INCONSISTENT" as const),
      resolvedAudit,
      handoffAudit,
      hasSummaries,
      summariesMatch
  ...
  if (status === "VALIDATED") {
    ...
    return {
      ...derived,
      mode: derived.ready && consistent ? ("READY" as const) : ("INCONSISTENT" as const),
      resolvedAudit,
      handoffAudit,
      hasSummaries,
      summariesMatch
