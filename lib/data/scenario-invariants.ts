...
export function assertSyntheticScenarioSeedInvariants(seed: SyntheticScenarioSeed): void {
  ...
  assert(
    seed.application.serviceHealth === "DEGRADED",
    "Initial application health must be DEGRADED."
  );
  assert(
  ...
  assert(
    seed.incident.selectedRemediation === null,
    "Reset incident must not pre-select remediation."
  );
  assert(seed.incident.recoveredRelease === null, "Reset incident must not claim recovery.");
  assert(
    seed.incident.createdAt.getTime() >
      new Date(SCENARIO.timeline.currentReleaseDeployedAt).getTime(),
  ...
  assert(
    seed.requests.length === 1,
    "The MVP scenario must contain exactly one failed API request."
  );
  assert(
    seed.transactions.length === 1,
    "The MVP scenario must contain exactly one failed synthetic transaction."
  );
  ...
  assert(
    ...
    "Request release version drifted."
  );
  assert(
    transaction.applicationVersion === SCENARIO.currentRelease.version,
    "Transaction release version drifted."
  );
  ...
  assert(
    correlatedLogs.some((log) => log.level === "ERROR"),
    "Timeout ERROR evidence is required."
  );
  ...
  for (const definition of REQUIRED_VALIDATION_DEFINITIONS) {
    ...
    assert(
      ...
      `Validation check ${definition.id} must reset to PENDING.`
    );
  }
