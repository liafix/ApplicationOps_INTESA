  ...
  return (
    <div className="relative grid gap-3 sm:grid-cols-[56px_1fr]">
      <div className="flex sm:flex-col sm:items-center">
        ...
        {index < 4 ? (
          <div className="ml-4 h-full min-h-6 w-px bg-slate-800 sm:ml-0 sm:mt-2" />
        ) : null}
      </div>
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h3 className="mt-2 text-lg font-semibold text-white">
              Follow one failure across release, request, logs and transaction state.
            </h3>
            <p className="mt-2 max-w-3xl text-xs leading-5 text-slate-400">
              These records are joined by synthetic correlation identifiers. A complete trail
              strengthens the investigation context, but correlation alone does not independently
              establish causation.
            </p>
          </div>
          <span
            className={`w-fit rounded-full border px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.14em] ${statusTone[correlation.status]}`}
          >
      ...
      <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
        <section
          className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4 sm:p-5"
          aria-label="Correlated event timeline"
        >
          <div className="flex items-end justify-between gap-3">
            <div>
              ...
              <h3 className="mt-1 text-sm font-semibold text-white">
                Persisted evidence in sequence
              </h3>
            </div>
        ...
        <aside
          className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4 sm:p-5"
          aria-label="Correlation integrity checks"
        >
          <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-400">
            Correlation integrity
          </p>
          <h3 className="mt-1 text-sm font-semibold text-white">
            Do the recorded evidence items agree?
          </h3>
          <div className="mt-3">
          ...
          <div className="mt-4 space-y-2">
            {correlation.correlatedLogs.length ? (
              correlation.correlatedLogs.map((log) => (
                <div
                  key={log.id}
                  className="rounded-xl border border-slate-800/80 bg-[#080d14] p-3"
                >
                  <div className="flex items-center justify-between gap-3">
                    <span
                      className={`font-mono text-[10px] font-semibold ${log.level === "ERROR" ? "text-rose-300" : log.level === "WARN" ? "text-amber-300" : "text-slate-400"}`}
                    >
                      {log.level}
                    </span>
                  ...
                  <p className="mt-2 font-mono text-[11px] leading-5 text-slate-300">
                    {log.message}
                  </p>
                </div>
              ))
              ...
              <p className="text-xs leading-5 text-amber-300">
                No logs were found for the canonical request ID.
              </p>
            )}
          </div>
        </article>
            ...
            <dl className="mt-4 space-y-3 font-mono text-xs">
              <div>
                ...
                <dd className="mt-1 text-slate-200">
                  {correlation.request.method} {correlation.request.path}
                </dd>
              </div>
              <div>
                <dt className="text-slate-500">request-id</dt>
                <dd className="mt-1 text-sky-300">{correlation.request.requestId}</dd>
              </div>
              <div>
                <dt className="text-slate-500">release</dt>
                <dd className="mt-1 text-slate-200">v{correlation.request.releaseVersion}</dd>
              </div>
              <div>
                <dt className="text-slate-500">response</dt>
                <dd className="mt-1 text-rose-300">
                  HTTP {correlation.request.responseStatus} · {correlation.request.durationMs} ms
                </dd>
              </div>
              <div>
                <dt className="text-slate-500">failure</dt>
                <dd className="mt-1 text-rose-300">{correlation.request.failureCode ?? "NONE"}</dd>
              </div>
            </dl>
          ) : (
            <p className="mt-4 text-xs leading-5 text-amber-300">
              Persisted request evidence is missing.
            </p>
          )}
        </article>
        ...
        <article className="rounded-2xl border border-slate-800 bg-slate-950/60 p-4 sm:p-5">
          ...
          <div className="mt-4 rounded-xl border border-slate-800 bg-[#080d14] p-3">
            <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500">
              Read-only diagnostic lookup · illustrative
            </p>
          ...
          {correlation.transaction ? (
            <dl className="mt-4 space-y-3 font-mono text-xs">
              <div>
                ...
                <dd className="mt-1 text-sky-300">{correlation.transaction.id}</dd>
              </div>
              <div>
                <dt className="text-slate-500">requestId</dt>
                <dd className="mt-1 text-sky-300">{correlation.transaction.requestId}</dd>
              </div>
              <div>
                <dt className="text-slate-500">status</dt>
                <dd className="mt-1 text-rose-300">{correlation.transaction.status}</dd>
              </div>
              <div>
                <dt className="text-slate-500">applicationVersion</dt>
                <dd className="mt-1 text-slate-200">
                  v{correlation.transaction.applicationVersion}
                </dd>
              </div>
              <div>
                <dt className="text-slate-500">failureCode</dt>
                <dd className="mt-1 text-rose-300">
                  {correlation.transaction.failureCode ?? "NONE"}
                </dd>
              </div>
            </dl>
          ) : (
            <p className="mt-4 text-xs leading-5 text-amber-300">
              Persisted transaction evidence is missing.
            </p>
          )}
        </article>
      </div>
