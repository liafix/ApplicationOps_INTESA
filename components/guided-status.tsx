...
export function GuidedIncidentStatusPill() {
  ...
  return (
    <span
      className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${statusTone[status] ?? statusTone.OPEN}`}
    >
      {status}
    </span>
  );
  ...
  if (step === 6)
    return (
      <span className="text-xs text-emerald-300">
        Walkthrough complete · incident safely resolved.
      </span>
    );
  return (
    <span className="text-xs text-slate-400">
      Stage {step} of 5 · Follow the evidence in sequence.
    </span>
  );
}
  ...
  if (!checks.length) {
    return (
      <p className="text-sm text-slate-400">
        Validation state is loading from the active demo scenario.
      </p>
    );
  }
  return (
    <div className="space-y-2">
      {checks.map((check) => (
        <div
          key={check.id}
          className="flex items-center justify-between gap-4 rounded-xl border border-slate-800 bg-slate-900/45 px-4 py-3"
        >
          <span className="text-sm text-slate-300">{check.label}</span>
          <span
            className={`rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${check.status === "PASS" ? "border-emerald-400/30 bg-emerald-400/10 text-emerald-200" : check.status === "FAIL" ? "border-rose-400/30 bg-rose-400/10 text-rose-200" : "border-slate-700 bg-slate-900 text-slate-300"}`}
          >
            {check.status}
          </span>
        </div>
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-950/55 p-5">
        ...
        <p className="mt-3 font-mono text-xs leading-6 text-slate-300">
          rootCause: {incident?.rootCauseCode ?? "PENDING"}
          <br />
          recoveredRelease: {incident?.recoveredRelease ?? "PENDING"}
          <br />
          validation: {passed}/4
          <br />
...
export function GuidedAuditTrail() {
  ...
  if (!audit.length)
    return (
      <p className="text-sm text-slate-400">
        Audit events are loading from the active demo scenario.
      </p>
    );
  return (
    <div className="space-y-1">
      {audit.map((event) => {
        ...
        return (
          <div
            key={event.id}
            className="grid gap-2 border-b border-slate-800/80 py-3 last:border-b-0 sm:grid-cols-[110px_1fr_auto] sm:items-center"
          >
...
export function GuidedIncidentMetricDetail() {
  ...
  return (
    <>
      {incident?.id ?? "APP-2047"} · {incident?.status ?? "OPEN"}
    </>
  );
}
